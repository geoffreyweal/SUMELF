...
This method has beem modifed from the isomorphvf2.py script from networkx by Geoffrey Weal

This version of the script contains a method called get_all_unique_isomorphic_graphs which will give all the unique ways that two graphs are isomorphic to each other.

This script does not require any external input from networkx. Networkx not needed as an input.
...
class GraphMatcher:
    ...
    def get_all_unique_isomorphic_graphs(self):
        """
        Returns all the unique isomorphic graphs.

        This method has been created by Geoff and is a modified version of the is_isomorphic method from the original networkx code. GRW

        The dictionary created will allow the user to turn graph 1 into graph 2.
        """
        ...
        # obtain all the isomorphic index graph representations between the two graphs. New by GRW.
        all_ico_counter = 0
        all_unique_icos = []
        for an_isos in self.isomorphisms_iter():
            all_ico_counter += 1
            if not an_isos in all_unique_icos:
                #yield an_isos
                all_unique_icos.append(an_isos)
    ...
    def syntactic_feasibility(self, G1_node, G2_node):
        ...
        try:
            if not (self.G1.nodes[G1_node] == self.G2.nodes[G2_node]):
                return False # GRW 9/8/22
        except Exception as exception:
            to_string  = 'You have the following error:\n\n'
