"""
obtain_new_bonding_unit_vectors_for_three_H_to_attach.py, Geoffrey Weal, 4/7/2022

This script is designed to obtain the new bonding_unit_vectors for adding three hydrogen atoms about an atom. 
"""
...
def obtain_new_bonding_unit_vectors_for_three_H_to_attach(bonding_unit_vectors, molecule, molecule_graph, index_to_attach_Hs_to, atom_element, number_of_lone_pairs_of_electrons, crystal, crystal_graph, element_to_attach=None, logger=None):
	"""
	This method is designed to obtain the new bonding_unit_vectors for adding three hydrogen atoms about an atom. 
	...
	new_bonding_unit_vectors : list of numpy.array 
		These are the bonding unit vectors to add new three hydrogens to in the current atom. 
	"""

	# Before beginning, note the number of new hydrogens to add to the system
	no_of_Hs_to_attach = 3
	...
	# Second, determine the method to use to obtain the new bonding unit vectors for adding three hydrogens to the central atom. 
	if   no_of_neighbouring_atoms_before_adding_hydrogens == 1:
		# There is three hydrogens being added to an atom with one other atom around it. We need the atom to have tetrahedral geometry.
		...
		# There is three hydrogens being added to an atom with no other atoms around it.
		...
		if   number_of_lone_pairs_of_electrons == 0:
			...
			# There is one lone pair of electrons. We need the atom to have tetrahedral geometry.
	...
	# Third, check that three new_bonding_unit_vector is given in the new_bonding_unit_vectors list
	if not (isinstance(new_bonding_unit_vectors,list) and (len(new_bonding_unit_vectors) == 3)):
