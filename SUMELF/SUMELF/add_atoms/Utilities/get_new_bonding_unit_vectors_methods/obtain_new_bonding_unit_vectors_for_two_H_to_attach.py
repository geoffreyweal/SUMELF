"""
obtain_new_bonding_unit_vectors_for_two_H_to_attach.py, Geoffrey Weal, 4/7/2022

This script is designed to obtain the new bonding_unit_vectors for adding two hydrogen atoms about an atom. 
"""
...
def obtain_new_bonding_unit_vectors_for_two_H_to_attach(bonding_unit_vectors, molecule, molecule_graph, index_to_attach_Hs_to, atom_element, number_of_lone_pairs_of_electrons, crystal, crystal_graph, element_to_attach=None, logger=None):
	"""
	This method is designed to obtain the new bonding_unit_vectors for adding two hydrogen atoms about an atom. 
	...
	new_bonding_unit_vectors : list of numpy.array 
		These are the bonding unit vectors to add two new hydrogens to in the current atom. 
	"""

	# Before beginning, note the number of new hydrogens to add to the system
	no_of_Hs_to_attach = 2
	...
	# Second, determine the method to use to obtain the new bonding unit vectors for adding two hydrogens to the central atom. 
	if   no_of_neighbouring_atoms_before_adding_hydrogens == 2:
		# There are two hydrogens being added to an atom with two other atoms around it. We need the atom to have tetrahedral geometry. 
		if not number_of_lone_pairs_of_electrons == 0:
			raise Exception('huh?')
		...
		# There is two hydrogens being added to an atom with one other atom around it.
		...
		if   number_of_lone_pairs_of_electrons == 0:
			...
			# There is one lone pairs of electrons. We need the atom to have tetrahedral geometry. 
		...
		# There is two hydrogens being added to an atom with no other atoms around it.

		if   number_of_lone_pairs_of_electrons == 0:
			# There are no lone pairs of electrons. We need the atom to have linear geometry.
			new_bonding_unit_vectors = add_two_neighbours_to_atom_with_no_current_neighbours(bonding_unit_vectors, molecule, molecule_graph, index_to_attach_Hs_to, atom_element, no_of_Hs_to_attach, number_of_lone_pairs_of_electrons, [], element_to_attach=element_to_attach, logger=logger)

		elif number_of_lone_pairs_of_electrons == 1:
			# There is one lone pair of electrons. We need the atom to have trigonal planar geometry.
			new_bonding_unit_vectors = add_three_neighbours_to_atom(bonding_unit_vectors, molecule, molecule_graph, index_to_attach_Hs_to, atom_element, no_of_Hs_to_attach, number_of_lone_pairs_of_electrons, crystal, crystal_graph, [], element_to_attach=element_to_attach, logger=logger)

		elif number_of_lone_pairs_of_electrons == 2:
			# There is two lone pairs of electrons. We need the atom to have tetrahedral geometry.
			new_bonding_unit_vectors = add_four_neighbours_to_atom(bonding_unit_vectors, molecule, molecule_graph, index_to_attach_Hs_to, atom_element, no_of_Hs_to_attach, number_of_lone_pairs_of_electrons, crystal, crystal_graph, [], element_to_attach=element_to_attach, logger=logger)

		else:
			raise Exception('huh?')

	# Third, check that two new_bonding_unit_vector is given in the new_bonding_unit_vectors list
	if not (isinstance(new_bonding_unit_vectors,list) and (len(new_bonding_unit_vectors) == 2)):
