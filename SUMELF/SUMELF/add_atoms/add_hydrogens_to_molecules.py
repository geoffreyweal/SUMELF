"""
add_hydrogens_to_crystal.py, Geoffrey Weal, 3/7/2022

This script is designed to automatically add hydrogens to missing sites if possible. 
"""
import os, warnings
...
def add_hydrogens_to_molecules(no_of_hydrogens_to_add_to_mols, molecules, molecule_graphs, original_crystal, original_crystal_graph, symmetry_operations, cell, molecule_name=None, logger=None, identifier=None, allow_hydrogen_free_rotation=False):
	"""
	This method is designed to add missing hydrogens to crystals. 

	These are hydrogens that have been recorded in the crystal but do not have coordinates. These hydrogens have been bonded to atoms in the crystal. 

	Parameters
	----------
	no_of_hydrogens_to_add_to_mols : dict. of dict. of {int: int}
		This dictionary contains the number of hydrogens you want to add to each atom in each molecule in the crystal. 
	...
	updated_molecule_graphs : list of networkx.graph
		These are the graphs associated with the molecules in the crystal. 
	were_hydrogens_added : bool.
		This boolean indicates if hydrogens were added to this system.
	"""
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	# Fourth, check that the atom indices and molecules given in no_of_hydrogens_to_add_to_mols are consistent with molecules
	
	# 4.1: Initalise a dictionary that contains which atoms in no_of_hydrogens_to_add_to_mols are missing in molecules
	inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules = {}

	# 4.2: For each molecule given in inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules:
	for mol_name, no_of_hydrogens_to_add_in_mol in no_of_hydrogens_to_add_to_mols.items():

		# 4.2.1: Check that the molecule given by mol_name exists in molecules
		if not mol_name in molecules.keys():
			inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules[mol_name] = 'No molecule'
			continue

		# 4.2.2: For each atom index in no_of_hydrogens_to_add_in_mol:
		for atom_index, no_of_hydrogens_to_add_to_atom in no_of_hydrogens_to_add_in_mol.items():

			#	# 4.2.2.1: Check that atom_index can be found in the molecule
			if   not (0 <= atom_index < len(molecules[mol_name])):
				inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.setdefault(mol_name,{})[atom_index] = 'Does not exist'

				# 4.2.2.2: Make sure that mol_name -> atom_index is not a hydrogen atom.
			elif molecules[mol_name][atom_index].symbol in ['H', 'D', 'T']:
				inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.setdefault(mol_name,{})[atom_index] = 'Is itself a hydrogen atom.'

				# 4.2.2.3: Make sure the user has entered an integer for number of hydrogens to add.
			elif not isinstance(no_of_hydrogens_to_add_to_atom,int):
				inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.setdefault(mol_name,{})[atom_index] = f'Input given not an integer => {no_of_hydrogens_to_add_to_atom}.'

				# 4.2.2.4: Make sure the user has not entered in 0 for number of hydrogen to add to atom, as this does not make any sense.
			elif no_of_hydrogens_to_add_to_atom == 0:
				inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.setdefault(mol_name,{})[atom_index] = 'Input given is 0.'

				# 4.2.2.5: Make sure the user has not entered a negative number for number of hydrogen to add to atom, as this does not make any sense.
			elif no_of_hydrogens_to_add_to_atom < 0:
				inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.setdefault(mol_name,{})[atom_index] = f'Input given is an integer number => {no_of_hydrogens_to_add_to_atom}.'

	# 4.3: If any problems are given in inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules, raise an Exception
	if len(inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules) > 0:
		to_string  = 'Error: Some of the molecules and/or atoms given in the "no_of_hydrogens_to_add_to_mols" dictionary have problems in this method.\n'
		to_string += 'These are:\n'
		to_string += '\n'
		to_string += 'molecule name\n'
		to_string += 'Atom index: Reason \n'
		to_string += '------------------\n'
		for mol_name, atom_indices_with_problems in inputs_in__no_of_hydrogens_to_add__that_are_missing_in_molecules.items():
	...
	# Eighth, initialise a boolean for noting if the crystal has been updated with extra hydrogens
	were_hydrogens_added = False

	# Ninth, initalise a list to record the indices of all the hydrogens added to the molecule. 
	all_atoms_with_added_hydrogens_indices = {}
	...
	for molecule_name in sorted(molecules.keys()):
		...
		# 10.3: Check that there are any hydrogens to add to molecule_name.
		#      * If there are no atoms to add to molecule_name, move on to the next molecule. 
		if molecule_name in no_of_hydrogens_to_add_to_mols.keys():

			# 10.3.1: Add the number of hydrogens you want to add to the molecule
			no_of_hydrogen_to_add_to_mol = no_of_hydrogens_to_add_to_mols[molecule_name]

			# 10.3.2: Determine if hydrogens need to be added to the molecule and if so, add hydrogens to the molecule.
			updated_molecule, updated_molecule_graph, were_hydrogens_added_to_this_molecule, atoms_with_added_hydrogens_indices = add_hydrogens_to_molecule(no_of_hydrogen_to_add_to_mol, molecule, molecule_graph, crystal, crystal_graph, molecule_name=molecule_name, allow_hydrogen_free_rotation=allow_hydrogen_free_rotation, logger=logger)

			# 10.3.3: Add either the original molecule or its updated form depending on if hydrogen were added to the molecule. 
			if not were_hydrogens_added_to_this_molecule: # If the molecule has been updated, do the following:
				to_string  = f'Error: No hydrogens were added to molecule {molecule_name} '
				if identifier is not None:
					to_string += f'(in crystal: {identifier})'
				to_string += '\n'
				to_string += f'This was unexpected, as the following hydrogens were to be added to this molecule: {no_of_hydrogen_to_add_to_mol}\n'
				to_string += 'Check this.\n'
				raise Exception(to_string)

			# Testing
			'''
			warnings.warn('adding molecules to METHOD_add_no_coord_hydrogens_to_molecules_test_folder for checking')
			test_folder = 'METHOD_add_no_coord_hydrogens_to_molecules_test_folder'
			...
			if (logger is not None) and (not were_hydrogens_added):
				logger.info('Found non-coordinated hydrogens to crystal structure of '+str(identifier))

			# 10.3.8: Set were_hydrogens_added to True, as one or more hydrogens have been added to the crystal. 
			were_hydrogens_added = True

		else: # Add the original molecule to updated_molecules

			# 10.3.9: Add the original molecule to updated_molecules.
			updated_molecules[molecule_name]       = molecule.copy()

			# 10.3.10: Add the original molecule graph to updated_molecule_graphs.
			updated_molecule_graphs[molecule_name] = deepcopy(molecule_graph)

	# Tenth, return updated_molecules, updated_molecule_graphs, and were_hydrogens_added.
	return updated_molecules, updated_molecule_graphs, were_hydrogens_added
