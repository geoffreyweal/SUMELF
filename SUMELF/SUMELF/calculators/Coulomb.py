...
class Coulomb(Calculator):
    """Coulombic potential calculator

    This class is designed to provide a calculator to replicate the Coulomb potential.
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        charges   = self.atoms.arrays['initial_charges']
        ...
        for ii in range(natoms):
            ...
            distance_vectors = positions[:] - positions[ii]
            distance_vectors = np.delete(distance_vectors, ii, axis=0)

            distances2 = (distance_vectors ** 2.0).sum(1)
            distances  =  distances2 ** 0.5

            # charge vectors
            Q_charge = charges[ii]
            q_charge = np.delete(charges, ii, axis=0)
            Qq = Q_charge * q_charge

            pairwise_energies =  Qq/distances
            pairwise_forces   = -Qq/distances2

            pairwise_forces = pairwise_forces[:, np.newaxis] * distance_vectors

            energies[ii] += pairwise_energies.sum()  # atomic energies
            forces[ii] += pairwise_forces.sum(axis=0)

        if len(self.fmax_list) == self.max_counter:
            del self.fmax_list[0]
        self.fmax_list.append(forces.max())
        self.fmax_com0_list = [round(fmax-self.fmax_list[0],6) for fmax in self.fmax_list]
        if (len(self.fmax_list) == self.max_counter) and all([(fmax_com0 == 0.0) for fmax_com0 in self.fmax_com0_list]):
            forces *= 0.0
        energy = 14.3996 * energies.sum()
...
class Repulsive_LennardJones(Calculator):
